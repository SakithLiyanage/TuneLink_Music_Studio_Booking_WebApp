...
import { FiUser, FiEdit, FiStar, FiCheckCircle } from 'react-icons/fi';
...
const ArtistProfile = () => {
  ...
  const [form, setForm] = useState({ bio: '', genres: '', hourlyRate: '', location: '', availability: '' });
  ...
  useEffect(() => {
    async function fetchProfile() {
      try {
        setLoading(true);
        const res = await artistsAPI.getById('me');
        setProfile(res.data.data);
        setForm({
          bio: res.data.data.longDescription || '',
          genres: (res.data.data.genres || []).join(', '),
  ...
  const handleSave = async (e) => {
    ...
    setSuccess(false);
    try {
      await artistsAPI.update(profile._id, {
        longDescription: form.bio,
      ...
      <div className="max-w-3xl mx-auto px-4">
        ...
        >
          <div className="flex items-center mb-8 gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-extrabold text-primary-700 mb-1">Artist Profile</h1>
              <p className="text-dark/60 text-lg">Update your artist information</p>
            </div>
          </div>
          <form onSubmit={handleSave} className="space-y-7">
            <motion.div variants={itemVariants}>
              <label className="block font-semibold mb-1">Bio</label>
              <textarea name="bio" value={form.bio} onChange={handleChange} className="w-full p-3 rounded-2xl border border-primary-100 bg-white/70 focus:ring-2 focus:ring-primary-400 focus:border-transparent transition-all" rows={3} />
            </motion.div>
            <motion.div variants={itemVariants}>
              <label className="block font-semibold mb-1">Genres <span className="text-xs text-dark/40">(comma separated)</span></label>
              <input name="genres" value={form.genres} onChange={handleChange} className="w-full p-3 rounded-2xl border border-primary-100 bg-white/70 focus:ring-2 focus:ring-primary-400 focus:border-transparent transition-all" />
            </motion.div>
            ...
            <motion.div variants={itemVariants}>
              ...
              <Link to="/dashboard/artist/availability" className="inline-block px-4 py-2 bg-primary-600 text-white rounded-xl hover:bg-primary-700 transition-all shadow-glass">Manage Availability</Link>
            </motion.div>
